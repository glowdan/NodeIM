...
class Cola
{
     ...
     * @internal param mixed $delimiter
     *
     * @return mixed
     */
    public static function getRootPath()
    {
        ...
        $rootUrl = 'http://' . $http_host;
        $pathArr = array_diff(explode(DIRECTORY_SEPARATOR, dirname(__FILE__)), explode("/", $DOCUMENT_ROOT));
        if (count($pathArr) > 1) {
            array_pop($pathArr);
            $app_path = implode(DIRECTORY_SEPARATOR, $pathArr);
            $appURL = $rootUrl . "/" . $app_path;
        } else {
            $appURL = $rootUrl;
        }
        $appURL = Cola::$config->get('_AppHome') ? $appURL . "/" . Cola::$config->get('_AppHome') : $appURL;
        return $appURL;
    }
}